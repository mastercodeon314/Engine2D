...
package com.engine2d.dsp.synth;
...
public abstract class Oscillator implements AudioProcessor
{
	...
	public Object[] process(AudioEvent audioEvent)
	{
		...
		{
			float[] next = generate(new float[audioEvent.getFloatBuffer().length]);
			//float[] last = audioEvent.getFloatBuffer();
			//float[] mix = Utils.mixSignals(last, next);
			audioEvent.setFloatBuffer(next);

			return new Object[] { true, next };
		}

		return new Object[] { false, null };
//		float[] buffer = new float[audioEvent.getFloatBuffer().length];
//		if (this.Enabled == true) generate(buffer);
//		audioEvent.setFloatBuffer(generate(audioEvent.getFloatBuffer())));
...
//		portStep = msStep / spms;
	...
	public final float[] generate(float[] signal)
	{
